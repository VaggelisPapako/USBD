...
typedef struct {
    bool is_heap;               // TRUE is heap file, FALSE is hash table
    BF_Block* first_block_id;   // pointer to FIRST block (where struct is stored)
    int max_records;            // max number of records that can be stored
    int id_last;                // id of the last record
    // int id_last_block;       // id of the last block // mhopws xreiazetai auto anti gia to panw
